...
bool lf_mpi_svc::init(){
    ...
    ret = _send_rtsp_frame_thread_ctx.session->AddSource(xop::MediaChannelId::channel_0,
...
void lf_mpi_svc::send_vi_frame_thread(send_vi_frame_thread_ctx * thread_ctx)
{
    ...
    bool res = rga_buf_ctor.create_buffer(
        svc->lf_config.width,
        svc->lf_config.height,
        rgba_format,
        addr_vi_rgba,
    ...
    while(!thread_ctx->stop_flag){
        if(thread_ctx->update_osd_f->load(std::memory_order_release)){
            osd_rgba_buffer = *thread_ctx->pixel_buffer;
            size_osd = *thread_ctx->pixel_buffer_size;
            thread_ctx->update_osd_f->store(false,std::memory_order_acquire);
        ...
        vi_mb_blk = svc->_send_rtsp_frame_thread_ctx.mpi_handle->vi_get_frame(&out_frame_info);
        if(vi_mb_blk){
            ...
            svc->_send_rtsp_frame_thread_ctx.mpi_handle->vi_release_frame();
            ...
            svc->_send_rtsp_frame_thread_ctx.mpi_handle->vi_send_frame(out_frame_info);
        }
    }
}
...
void lf_mpi_svc::osd_update_timer_thread(osd_update_timer_thread_ctx *thread_ctx)
{
    ...
    time_t raw_time = 0;
    std::string local_time_str('\0',128);
    time_t next_minute = 0;
    while(!thread_ctx->stop_flag->load(std::memory_order_release)){
        if(!thread_ctx->update_osd_f) //cleared by send_vi_frame_thread
        {
            ...
            thread_ctx->update_osd_f->store(true,std::memory_order_acquire);
...
void lf_mpi_svc::connect_callback(xop::MediaSessionId sessionId, std::string peer_ip, 
    ...
    lf_mpi_svc::idr_reset.store(true,std::memory_order_release);
    lf_mpi_svc::client_conn_flag.store(true,std::memory_order_release);
}

void lf_mpi_svc::disconnect_callback(xop::MediaSessionId sessionId, std::string peer_ip, 
                                        uint16_t peer_port){
    lf_mpi_svc::client_conn_flag.store(false,std::memory_order_release);
}
